...
const formatTimeDifference = (dateString: string) => {
  ...
  const diffInSeconds = Math.floor(
    (currentDate.getTime() - uploadDate.getTime()) / 1000
  );
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Proposal Documents for {student.name}</DialogTitle>
          <DialogDescription>
            Uploaded Proposal Documents with Supervisor:{" "}
        ...
        <div className="space-y-4">
              ...
              <CardContent className="p-4">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">
                    Uploaded{" "}
                    {formatTimeDifference(documentsToShow[0].uploadedAt)}
                  </span>
                </div>
          ...
          {onReview && (
            <div className="mt-4 flex justify-end space-x-2">
              <Button variant="destructive" onClick={() => onReview()}>
                Review Proposal
              </Button>
            </div>
          )}
