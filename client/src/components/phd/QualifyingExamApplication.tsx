...
const QualifyingExamApplication: React.FC<QualifyingExamApplicationProps> = ({
  ...
  const submitApplicationMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      const errorMessage = (error as ErrorWithResponse)?.response?.data?.message || "Failed to submit application";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!files.qualifyingArea1Syllabus || !files.qualifyingArea2Syllabus || 
        !files.tenthReport || !files.twelfthReport || !files.undergradReport) {
      toast.error("Please upload all required files. Only masters report is optional.");
      return;
    }
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{formatDate(exam.submissionDeadline)}</p>
            </div>
            <div>
            ...
            {exam.vivaDate && (
              <div>
                ...
                <p className="text-sm text-gray-600">{formatDate(exam.vivaDate)}</p>
              </div>
            )}
          </div>
        </CardContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, qualifyingArea1: e.target.value }))}
                ...
                >
                  ...
                  {subAreas.map((area) => (
                    <option key={area.subAreaName} value={area.subAreaName}>
                      {area.subAreaName}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, qualifyingArea2: e.target.value }))}
                  className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                  required
                >
                  ...
                  {subAreas.map((area) => (
                    <option 
                      key={area.subAreaName} 
                      value={area.subAreaName}
                      disabled={area.subAreaName === formData.qualifyingArea1}
                    >
                      {area.subAreaName}
                    </option>
                  ))}
              ...
              {[
                ...
                <div key={fileField.key}>
                  ...
                  {files[fileField.key as keyof FileFields] ? (
                    <div className="mt-1 space-y-2">
                      <div className="flex items-center justify-between gap-2 rounded-md border bg-gray-50 p-3">
                        <span className="text-sm text-gray-700">
                          {(files[fileField.key as keyof FileFields] as File).name}
                        </span>
                        <Button
                          variant="outline"
                          type="button"
                          onClick={() => handleFileChange(fileField.key as keyof FileFields, null)}
                      ...
                      {files[fileField.key as keyof FileFields] && (
                        <div className="relative mt-2 w-full">
                          <iframe
                            src={URL.createObjectURL(files[fileField.key as keyof FileFields] as File)}
                    ...
                    <FileUploader
                      value={[]}
                      onValueChange={(val) => handleFileChange(fileField.key as keyof FileFields, val[0] || null)}
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {fileField.required ? "Required" : "Optional"} - Upload in PDF format only
                  </p>
                </div>
              ))}
            </div>
