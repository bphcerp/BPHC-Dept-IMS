...
const DrcProposalManagement: React.FC = () => {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const seminarPendingIds =
        proposals
          ?.filter((p) => p.status === "seminar_details_pending")
          .map((p) => p.id) ?? [];
      setSelectedProposalIds(seminarPendingIds);
    } else {
      setSelectedProposalIds([]);
    }
  };
  const semesterDeadlines = proposals?.[0]?.proposalSemester;
  const proposalsReadyForNotice = proposals?.filter(
    (p) => p.status === "seminar_details_pending"
  );
  return (
      ...
      <Card>
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div>
            ...
            <CardDescription>
              {" "}
              Select proposals with status &quot;SEMINAR DETAILS PENDING&quot;
                  ...
                  <TableRow key={proposal.id}>
                    ...
                    <TableCell>
                      ...
                      <Checkbox
                        ...
                        disabled={proposal.status !== "seminar_details_pending"}
                      />{" "}
                    </TableCell>{" "}
                    <TableCell>
                      {" "}
