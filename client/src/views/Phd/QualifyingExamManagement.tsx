...
const QualifyingExamManagement: React.FC = () => {
  ...
  // Update application status mutation - REMOVED as status actions are now verify/resubmit
  ...
  // Statistics - Updated for new status types
  const totalApplications = applications.length;
  const pendingApplications = applications.filter(app => app.status === "applied").length;
  const verifiedApplications = applications.filter(app => app.status === "verified").length;
  const resubmitApplications = applications.filter(app => app.status === "resubmit").length;
            ...
            <div>
              ...
              <Select onValueChange={handleExamSelect} disabled={isLoadingExams}>
                ...
                <SelectContent className="max-h-60">
                  {exams.map((exam) => (
                    <SelectItem key={exam.id} value={exam.id.toString()} className="py-3">
                ...
                <Card>
                  <CardContent className="flex items-center p-6">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">{verifiedApplications}</p>
                ...
                <Card>
                  <CardContent className="flex items-center p-6">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">{resubmitApplications}</p>
                        <p className="text-sm text-gray-600">Needs Resubmit</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
