...
export default function AssignGradesView() {
    ...
    const [userRoles, setUserRoles] = useState<{ instructor: boolean; supervisor: boolean }>({ instructor: false, supervisor: false });
    const [instructorAssignments, setInstructorAssignments] = useState<InstructorAssignment[]>([]);
    const [coursePhase, setCoursePhase] = useState<'draft' | 'midsem' | 'endsem'>('draft');

    const loadCoursePhase = useCallback((courseName: string) => {
        // Get the phase from any grade record for this course
        const gradeForCourse = Object.values(grades).find(g => g.courseName === courseName && g.role === 'instructor');
        console.log(`loadCoursePhase for ${courseName}:`, {
            gradeForCourse,
            phase: gradeForCourse?.phase,
            allGrades: Object.values(grades).filter(g => g.courseName === courseName)
        });
        if (gradeForCourse && gradeForCourse.phase) {
            setCoursePhase(gradeForCourse.phase);
        } else {
            setCoursePhase('draft');
        }
    }, [grades]);
    ...
    useEffect(() => {
        void (async () => {
            try {
                ...
                const res = await api.get<{ data: { students: StudentRow[]; grades: GradeRow[]; targetCourses: string[]; courses: string[]; instructorAssignments: InstructorAssignment[] } }>(`/grades/supervisor`);
                ...
                setGrades(allGrades);
                setUserRoles({ instructor: hasInstructorRole, supervisor: hasSupervisorRole });
                ...
                const instructorCourses = [...new Set((data.instructorAssignments || []).map((ia: InstructorAssignment) => ia.courseName))];
                const allAvailableCourses = [...new Set([...allCourses, ...instructorCourses])];
    ...
    useEffect(() => {
        if (activeCourse) {
            loadCoursePhase(activeCourse);
        }
    }, [activeCourse, grades, loadCoursePhase]);
    ...
    const rows = useMemo<Array<{
        ...
        const filteredStudents = students.filter((s) => {
            ...
            const isInstructorAssigned = instructorAssignments.some((ia: InstructorAssignment) =>
                ia.studentErpId === s.erpId && ia.courseName === activeCourse
            );

            // Check if this student is assigned as instructor for ANY course
            const isInstructorAssignedAnywhere = instructorAssignments.some((ia: InstructorAssignment) =>
                ia.studentErpId === s.erpId
            );
        ...
        return filteredStudents
            .map((s) => {
                ...
                if (g) {
                    ...
                    const instructorAssignment = instructorAssignments.find((ia: InstructorAssignment) =>
                        ia.studentErpId === s.erpId && ia.courseName === activeCourse
                    );
    ...
    const getGradeColumnRole = useCallback((sheetName: string, headerIndex: number): 'midsem' | 'endsem' | null => {
    ...
    const saveRow = async (studentErpId: string, role: 'instructor' | 'supervisor') => {
        setIsSaving(true);
        try {
            const key = `${studentErpId}::${activeCourse}`;
            const row = grades[key];
            const endpoint = role === 'instructor' ? '/grades/submit-grades' : '/grades/supervisor/save';
    ...
    const uploadDoc = async (studentErpId: string, file: File, type: 'mid' | 'end', erpId?: string | null, name?: string | null) => {
        ...
        try {
            ...
            if (fileId) {
                setGrades((prev) => {
                    const key = `${studentErpId}::${activeCourse}`;
                    const existing: Partial<GradeRow> = prev[key] || ({ studentErpId, courseName: activeCourse } as Partial<GradeRow>);
        ...
        <div className="container mx-auto p-6 max-w-7xl">
            <div className="mb-6">
                ...
                <div className="flex gap-2 mt-2">
                    {userRoles.instructor && <Badge variant="secondary">Instructor</Badge>}
                    ...
                    <Tabs value={activeCourse} onValueChange={setActiveCourse}>
                        <TabsList className="inline-flex h-auto items-center justify-start rounded-md bg-muted p-1 text-muted-foreground overflow-x-auto w-full scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                            {(() => {
                                ...
                                const instructorCourses = [...new Set(instructorAssignments.map((ia: InstructorAssignment) => ia.courseName))];
                                const excelCourses = sheets.length ? sheets.map(s => s.sheetName) : [];
                                const allCourses = [...new Set([...excelCourses, ...instructorCourses])];

                                // If no courses from Excel or instructor assignments, use defaults only for supervisors
                                if (allCourses.length === 0 && userRoles.supervisor && !userRoles.instructor) {
                                            ...
                                            <TableHeader>
                                                <TableRow>
                                                    ...
                                                    {!sheets.find(s => s.sheetName === c) && (
                                                        <>
                                                            <TableHead className="min-w-[120px]">ERP ID</TableHead>
                                                            <TableHead className="min-w-[120px]">Campus ID</TableHead>
                                                            <TableHead className="min-w-[120px]">Name</TableHead>
                                                            <TableHead className="min-w-[100px]">Midsem Marks</TableHead>
                                                            <TableHead className="min-w-[120px]">Midsem Grade</TableHead>
                                                            <TableHead className="min-w-[100px]">Endsem Marks</TableHead>
                                                            <TableHead className="min-w-[120px]">Endsem Grade</TableHead>
                                                ...
                                                {rows.map((r) => (
                                                    <TableRow key={r.email}>
                                                        {(sheets.find(s => s.sheetName === c)?.columnHeaders || []).map((header, headerIndex) => {
                                                            ...
                                                            const isInstructorColumn = header.name.toLowerCase().includes('instructor') ||
                                                            ...
                                                            if (isInstructorColumn) {
                                                                ...
                                                                const assignedInstructor = instructorAssignments.find((ia: InstructorAssignment) =>
                                                                    ia.studentErpId === r.erpId && ia.courseName === c
                                                                );
                                                                const instructorName = assignedInstructor?.instructorName || "Not assigned";
                                                                ...
                                                                return (
                                                                    <TableCell key={header.name}>
                                                                        <Input type="number" value={(() => { const vv = r.excelRow[header.name]; return String((typeof vv === 'string' || typeof vv === 'number') ? vv : ""); })()} onChange={(e) => {
                                                                            ...
                                                                            const gradeRole = getGradeColumnRole(c, headerIndex + 1);
                                                                            if (gradeRole === 'midsem') {
                                                                                grades[gradeKey] = { ...grades[gradeKey], midsemMarks: v === "" ? undefined : Number(v) } as GradeRow;
                                                                            } else if (gradeRole === 'endsem') {
                                                                                grades[gradeKey] = { ...grades[gradeKey], endsemMarks: v === "" ? undefined : Number(v) } as GradeRow;
                                                                            }

                                                                            forceRerender();
                                                                        }} className="w-28" placeholder="Marks" readOnly={coursePhase === 'draft' || (coursePhase === 'midsem' && getGradeColumnRole(c, headerIndex + 1) === 'endsem') || (coursePhase === 'endsem' && getGradeColumnRole(c, headerIndex + 1) === 'midsem')} />
                                                                    </TableCell>
                                                                );
                                                            }
                                                            if (header.type === 'grade') {
                                                                ...
                                                                const options = fixedOptions ?? ((sheetOptions && sheetOptions.length > 0) ? sheetOptions : null);
                                                                ...
                                                                const savedGrade = (() => {
                                                                    ...
                                                                    const gradeRole = getGradeColumnRole(c, headerIndex);
                                                                    if (gradeRole === 'midsem') return g.midsemGrade || undefined;
                                                                    if (gradeRole === 'endsem') return g.compreGrade || undefined;
                                                                    ...
                                                                    return (
                                                                        <TableCell key={header.name}>
                                                                            <Select value={String(current)} onValueChange={(v) => {
                                                                                ...
                                                                                const gradeRole = getGradeColumnRole(c, headerIndex);
                                                                                if (gradeRole === 'midsem') {
                                                                                    grades[gradeKey] = { ...grades[gradeKey], midsemGrade: v } as GradeRow;
                                                                                } else if (gradeRole === 'endsem') {
                                                                                    grades[gradeKey] = { ...grades[gradeKey], compreGrade: v } as GradeRow;
                                                                                }

                                                                                forceRerender();
                                                                            }} disabled={coursePhase === 'draft' || (coursePhase === 'midsem' && getGradeColumnRole(c, headerIndex) === 'endsem') || (coursePhase === 'endsem' && getGradeColumnRole(c, headerIndex) === 'midsem')}>
                                                                ...
                                                                return (
                                                                    <TableCell key={header.name}>
                                                                        <Input value={String(current)} onChange={(e) => {
                                                                            r.excelRow[header.name] = e.target.value;

                                                                            // Also update grades state for saving
                                                                            const gradeKey = `${r.erpId}::${c}`;
                                                                            const gradeRole = getGradeColumnRole(c, headerIndex);
                                                                            if (gradeRole === 'midsem') {
                                                                                grades[gradeKey] = { ...grades[gradeKey], midsemGrade: e.target.value } as GradeRow;
                                                                            } else if (gradeRole === 'endsem') {
                                                                                grades[gradeKey] = { ...grades[gradeKey], compreGrade: e.target.value } as GradeRow;
                                                                            }

                                                                            forceRerender();
                                                                        }} placeholder="Enter grade" className="w-40" readOnly={coursePhase === 'draft' || (coursePhase === 'midsem' && getGradeColumnRole(c, headerIndex) === 'endsem') || (coursePhase === 'endsem' && getGradeColumnRole(c, headerIndex) === 'midsem')} />
                                                                    </TableCell>
                                                                );
                                                            }
                                                        ...
                                                        {!sheets.find(s => s.sheetName === c) && (
                                                            <>
                                                                ...
                                                                <TableCell>
                                                                    <div className="font-medium">{r.campusId || "—"}</div>
                                                                </TableCell>
                                                                <TableCell>
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        onChange={(e) => {
                                                                            setGrades(prev => {
                                                                                ...
                                                                                updated[key] = {
                                                                                    ...updated[key],
                                                                                    midsemMarks: e.target.value === "" ? undefined : Number(e.target.value),
                                                                                } as GradeRow;
                                                                                return updated;
                                                                            });
                                                                        }}
                                                                        placeholder="Marks"
                                                                        className="w-28"
                                                                        readOnly={coursePhase === 'draft' || coursePhase === 'endsem'}
                                                                    />
                                                                </TableCell>
                                                                <TableCell>
                                                                    <Input
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        onChange={(e) => {
                                                                            setGrades(prev => {
                                                                                ...
                                                                                updated[key] = {
                                                                                    ...updated[key],
                                                                                    endsemMarks: e.target.value === "" ? undefined : Number(e.target.value),
                                                                                } as GradeRow;
                                                                                return updated;
                                                                            });
                                                                        }}
                                                                        placeholder="Marks"
                                                                        className="w-28"
                                                                        readOnly={coursePhase === 'draft' || coursePhase === 'midsem'}
                                                                    />
                                                                </TableCell>
                                                                <TableCell>
                                                                    <Input
                                                        ...
                                                        <TableCell>
                                                            <div className="flex items-center gap-2">
                                                                <Input type="file" accept="application/pdf" onChange={(e) => { const f = e.target.files?.[0]; if (f) void uploadDoc(r.erpId || "", f, 'mid', r.erpId, r.name); }} disabled={coursePhase === 'draft' || coursePhase === 'endsem'} />
                                                                {r.midsemDocFileId ? (
                                                        ...
                                                        <TableCell>
                                                            <div className="flex items-center gap-2">
                                                                <Input type="file" accept="application/pdf" onChange={(e) => { const f = e.target.files?.[0]; if (f) void uploadDoc(r.erpId || "", f, 'end', r.erpId, r.name); }} disabled={coursePhase === 'draft' || coursePhase === 'midsem'} />
                                                                {r.endsemDocFileId ? (
                                                        ...
                                                        <TableCell>
                                                            <Button size="sm" onClick={() => { void saveRow(r.erpId || "", r.role); }} disabled={isSaving || coursePhase === 'draft'}>Save</Button>
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                                            </TableBody>
