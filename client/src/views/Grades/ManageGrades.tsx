...
export default function ManageGrades() {
    ...
    const [instructorColumnInfo, setInstructorColumnInfo] = useState<InstructorColumnInfo[]>([]);
    ...
    const loadFacultyList = useCallback(async (search = "") => {
        try {
            const response = await api.get<{ success: boolean; data: FacultyMember[] }>(`/grades/faculty-list?search=${encodeURIComponent(search)}&limit=100`);
    ...
    const loadExistingInstructorAssignments = useCallback(async () => {
        ...
        try {
            ...
            excelData.sheets.forEach(sheet => {
                ...
                const erpCandidates = ["ERP ID", "ERP", "EMPLID", "Campus ID", "ID", "Reg No", "Registration"];
                const erpHeader = sheet.columnHeaders?.find(h =>
                    erpCandidates.some(c => h.name.toLowerCase().includes(c.toLowerCase()))
                );
                if (erpHeader) {
            ...
            const response = await api.get<{ success: boolean; data: Array<{ studentErpId: string; courseName: string; instructorEmail: string; instructorName: string; phase?: string }> }>(`/grades/instructor-assignments?${params.toString()}`);
    ...
    const assignStudentInstructor = async (studentErpId: string, courseName: string, instructorEmail: string, campusId?: string, studentName?: string) => {
        try {
            const response = await api.post<{ success: boolean; message: string }>("/grades/assign-student-instructor", {
                studentErpId,
                courseName,
                instructorEmail,
                campusId,
            ...
            if (response.data.success) {
                ...
                const selectedInstructor = facultyList.find(f => f.email === instructorEmail);
                if (selectedInstructor) {
                    const instructorName = selectedInstructor.name || selectedInstructor.email;
    ...
    const sendMidsemNotification = async () => {
        ...
        try {
            ...
            const response = await api.post<{ success: boolean; message: string }>("/grades/notifications/send-midsem", {
                courseNames,
                subject: midsemNotificationSent ? `Midsem Grades Reminder` : `Midsem Grades Due`,
    ...
    const sendEndsemNotification = async () => {
        ...
        try {
            ...
            const response = await api.post<{ success: boolean; message: string }>("/grades/notifications/send-endsem", {
                courseNames,
                subject: endsemNotificationSent ? `Endsem Grades Reminder` : `Endsem Grades Due`,
        ...
        try {
            ...
            setModifiedData((prev) => {
                ...
                cloned.sheets.forEach((sheet) => {
                    ...
                    const linkByErp: Record<string, { mid?: number; end?: number; topic?: string }> = {};
                    ref.rows.forEach((row) => {
                        ...
                        const topic = (row as Record<string, unknown>)["_topic"] as string | undefined;
                        if (mid || end || topic) {
                            if (typeof erp === 'string' || typeof erp === 'number') linkByErp[String(erp)] = { mid, end, topic };
                        }
                    });
                    sheet.studentData.forEach((row) => {
                        ...
                        if ((typeof erp === 'string' || typeof erp === 'number') && linkByErp[String(erp)]) {
                            ...
                            if (links.topic) (row as Record<string, unknown>)["_topic"] = links.topic;
                        }
                    });
                });
    ...
    const mergeSupervisorGrades = useCallback(async (data: ExcelData) => {
        try {
            ...
            const res = await api.get<{ data: { gradesMap: Record<string, { midsemGrade?: string; compreGrade?: string; midsemMarks?: number; endsemMarks?: number; topic?: string; midsemDocFileId?: number; endsemDocFileId?: number }>; totalGrades: number } }>(`/grades/public/get-all-saved-grades?${params.toString()}`);
    ...
    useEffect(() => {
        const navState = location.state as unknown as { excelData?: ExcelData; instructorColumnInfo?: InstructorColumnInfo[] } | null;
        if (navState?.excelData) {
            ...
            try {
                ...
                if (navState.instructorColumnInfo) {
                    localStorage.setItem("grades:instructorColumnInfo", JSON.stringify(navState.instructorColumnInfo));
                }
        ...
        try {
            ...
            const savedColumnInfo = localStorage.getItem("grades:instructorColumnInfo");
            if (savedData) {
                const parsedData = JSON.parse(savedData) as ExcelData;
                setExcelData(parsedData);
                setModifiedData(parsedData);
                if (savedColumnInfo) {
                    const parsedColumnInfo = JSON.parse(savedColumnInfo) as InstructorColumnInfo[];
                ...
                <CardContent>
                    <div className="flex gap-2">
                        <Button
                            ...
                            variant="outline"
                        >
                            {midsemNotificationSent ? 'Send Midsem Reminder' : 'Send Midsem Notification'} (All Courses)
                        </Button>
                        <Button
                            ...
                            variant="outline"
                        >
                            {endsemNotificationSent ? 'Send Endsem Reminder' : 'Send Endsem Notification'} (All Courses)
                        </Button>
                    </div>
                </CardContent>
            </Card>
                                                        ...
                                                        <TableRow key={rowKey}>
                                                            {(sheet.columnHeaders?.filter((h) => {
                                                                ...
                                                                <TableCell key={header.name}>
                                                                    {(() => {
                                                                        const sheetInfo = instructorColumnInfo.find(info => info.sheetName === sheet.sheetName);
                                                                        const isInstructorColumn = sheetInfo?.hasInstructorColumn &&
                                                                            sheetInfo.instructorColumnName === header.name;
                                                                        ...
                                                                        if (isInstructorColumn) {
                                                                            ...
                                                                            const studentErpId = erpHeader ? student?.[erpHeader.name] : null;
                                                                            const instructorKey = `${studentErpId}::${sheet.sheetName}`;
                                                                            const selectedInstructorName = selectedInstructors[instructorKey];
                                                                            const isOpen = openDropdowns[instructorKey] || false;

                                                                            // Find campus ID and name from Excel columns
                                                                            const campusIdHeader = sheet.columnHeaders?.find(h =>
                                                                                h.name.toLowerCase().includes('campus') && h.name.toLowerCase().includes('id')
                                                                            );
                                                                            const nameHeader = sheet.columnHeaders?.find(h =>
                                                                                h.name.toLowerCase() === 'name' ||
                                                                                h.name.toLowerCase().includes('student name')
                                                                            );
                                                                            const campusId = campusIdHeader ? student?.[campusIdHeader.name] : null;
                                                                            const studentName = nameHeader ? student?.[nameHeader.name] : null;
                                                                            ...
                                                                            return (
                                                                                <Popover
                                                                                    open={isOpen}
                                                                                    onOpenChange={(open) => setOpenDropdowns(prev => ({ ...prev, [instructorKey]: open }))}
                                                                                >
                                                                                    <PopoverTrigger asChild>
                                                                                        ...
                                                                                        >
                                                                                            {selectedInstructorName || "Select instructor..."}
                                                                                                ...
                                                                                                <CommandGroup>
                                                                                                    {facultyList.map((faculty) => (
                                                                                                        <CommandItem
                                                                                                            ...
                                                                                                            onSelect={() => {
                                                                                                                void assignStudentInstructor(String(studentErpId), sheet.sheetName, faculty.email, String(campusId), String(studentName));
                                                                                                                setOpenDropdowns(prev => ({ ...prev, [instructorKey]: false }));
                                                                                                            }}
                                                                                                        >
                                                                                                            <Check
                                                                                                                className={`mr-2 h-4 w-4 ${selectedInstructorName === faculty.name ? "opacity-100" : "opacity-0"
                                                                                                                    }`}
                                                                                                            />
                                                            ...
                                                            <TableCell>
                                                                <div className="font-medium">
                                                                    {(() => {
                                                                        const topic = (student as Record<string, unknown>)["_topic"];
