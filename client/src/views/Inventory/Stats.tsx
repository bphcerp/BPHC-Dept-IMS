...
const checkDateStatus = (
  ...
  const isExpiringSoon =
    isBefore(targetDate, addDays(now, daysThreshold)) && targetDate > now;
  const isExpired =
    isBefore(targetDate, now) ||
      ...
      <div className="flex w-3/4 flex-col space-y-4 p-4">
        ...
        <div className="mb-4">
          ...
          >
            ...
            <SelectContent>
              {checkAccess(permissions["/inventory/stats/lab-year"]) && (
                <SelectItem value="inventory/stats/lab-year">
                  Lab Sum Per Year
                </SelectItem>
              )}
              {checkAccess(permissions["/inventory/stats/lab-category"]) && (
                <SelectItem value="inventory/stats/lab-category">
                  Lab Sum Per Category
                </SelectItem>
              )}
              {checkAccess(permissions["/inventory/stats/vendor-year"]) && (
                <SelectItem value="inventory/stats/vendor-year">
                  Vendor Sum Per Year
                </SelectItem>
              )}
              {checkAccess(permissions["/inventory/vendors/get"]) && (
                <SelectItem value="inventory/vendors/get">
                  Vendor Categories
                </SelectItem>
              )}
            </SelectContent>
        ...
        {selectedStat === "inventory/stats/lab-year" && isSuccess ? (
          <LabStatsPerYear data={statData} />
        ) : (
          <></>
        )}
        {selectedStat === "inventory/stats/lab-category" && isSuccess ? (
          <LabStatsPerCategory data={statData} />
        ) : (
          <></>
        )}
        {selectedStat === "inventory/stats/vendor-year" && isSuccess ? (
          <VendorStatsPerYear data={statData} />
        ) : (
          <></>
        )}
        {selectedStat === "inventory/vendors/get" && isSuccess ? (
          <VendorStatsCategories
            data={statData}
        ...
        {!selectedStat && (
          <p className="text-sm text-gray-400">
            Please select a statistic to view the data
          </p>
        )}
      </div>
      <div className="h-screen w-1/4 overflow-y-auto rounded-md px-4">
        <h3 className="sticky left-0 top-0 mb-4 bg-background p-4 text-center text-lg font-semibold underline">
        ...
        {importantDates.length > 0 ? (
          <ul className="space-y-2">
            {importantDates.map((item) => {
              const {
                isExpiringSoon: warrantyExpiring,
                isExpired: warrantyExpired,
              } = checkDateStatus(item.warrantyTo, 7);
              const { isExpiringSoon: amcExpiring, isExpired: amcExpired } =
                checkDateStatus(item.amcTo, 7);
                  ...
                  <Card className="shadow-sm">
                    ...
                    <CardContent className="text-sm text-foreground">
                      <p className="text-xs text-muted-foreground">
                        <strong>Equipment ID:</strong> {item.equipmentID}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        <strong>Lab:</strong> {item.lab?.name}
                      </p>
                      <p
                        className={`${warrantyExpired ? "text-red-500 line-through" : warrantyExpiring ? "font-semibold text-yellow-500" : "text-xs text-muted-foreground"}`}
                      >
                        <strong>Warranty To:</strong>{" "}
                        {item.warrantyTo
                          ? `${new Date(item.warrantyTo).toLocaleDateString()} (${hdate.relativeTime(new Date(item.warrantyTo))})`
                          : "N/A"}
                      </p>
                      <p
                        className={`${amcExpired ? "text-red-500 line-through" : amcExpiring ? "font-semibold text-yellow-500" : "text-xs text-muted-foreground"}`}
                      >
                        <strong>AMC To:</strong>{" "}
                        {item.amcTo
                          ? `${new Date(item.amcTo).toLocaleDateString()} (${hdate.relativeTime(new Date(item.amcTo))})`
                          : "N/A"}
                      </p>
                    </CardContent>
                  </Card>
