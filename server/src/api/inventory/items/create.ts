...
import { inventoryItems, laboratories, inventoryCategories } from "@/config/db/schema/inventory.ts";
...
router.post('/', checkAccess(), asyncHandler(async (req, res) => {
    try {
        const parsed = inventoryItemSchema.omit({ id: true, transferId: true, serialNumber: true }).parse({ ...req.body, poAmount: (req.body.poAmount as number).toString() });
        ...
        // Get lab code
        const labCodeResult = await db.select({ code: laboratories.code }).from(laboratories).where(eq(laboratories.id, labId)).limit(1);
        const labCode = labCodeResult[0]?.code;

        // Get category code
        const categoryCodeResult = await db.select({ code: inventoryCategories.code }).from(inventoryCategories).where(eq(inventoryCategories.id, itemCategoryId)).limit(1);
