...
export default router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const suggestionsArea1 = await db.query.phdExaminerSuggestions.findFirst({
      where: and(
        eq(phdExaminerSuggestions.applicationId, applicationId),
        eq(phdExaminerSuggestions.qualifyingArea, application.qualifyingArea1),
      ),
    });

    const suggestionsArea2 = await db.query.phdExaminerSuggestions.findFirst({
      where: and(
        eq(phdExaminerSuggestions.applicationId, applicationId),
        eq(phdExaminerSuggestions.qualifyingArea, application.qualifyingArea2),
      ),
    });
    ...
    res
      .status(200)
      .json({ success: true, message: "Examiners assigned successfully" });
  }),
);