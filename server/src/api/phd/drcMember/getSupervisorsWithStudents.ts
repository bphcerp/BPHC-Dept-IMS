...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    const supervisorEmails = [...new Set(students.map(student => student.supervisorEmail))].filter(email => email !== null);
    ...
    res.status(200).json({ success: true, supervisors: supervisorsWithStudents });
  })
);
