...
router.get(
    ...
    asyncHandler(async (req, res, next) => {
        ...
        const activeExam = await db
            ...
            .where(
                and(
                    gte(phdQualifyingExams.deadline, currentDate),
        ...
        threeMonthsBeforeDeadline.setMonth(
            threeMonthsBeforeDeadline.getMonth() - 3
        );

        // Check if we're generating form data
        const { studentEmails, examStartDate, examEndDate, roomNumber } =
            req.query;
