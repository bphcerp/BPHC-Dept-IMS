...
export default router.get(
  ...
  asyncHandler(async (_req, res, next) => {
    ...
    if (!semesters.length) {
      return next(new HttpError(HttpCode.NOT_FOUND, "No semesters found"));
    }
          ...
          exams.map(async (exam) => {
            ...
            const studentsWithDocuments = await Promise.all(
              students.map(async (student) => {
                ...
                const qualifyingAreas: Record<string, string | null> = {
                ...
                textFieldsData.forEach((field) => {
                  if (field.fieldName) {
                    qualifyingAreas[field.fieldName as keyof typeof qualifyingAreas] = field.value;
                  }
                });
