  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (!examId || isNaN(examId)) {
        return next(new HttpError(HttpCode.BAD_REQUEST, "Valid exam ID is required"));
      }
      ...
      if (!exam) {
        return next(new HttpError(HttpCode.NOT_FOUND, "Qualifying exam not found"));
      }
      ...
      if (studentsWithRecentApp.length === 0) {
        return next(new HttpError(HttpCode.NOT_FOUND, "No applications found for this exam"));
      }
          ...
          if (qualificationForm.length > 0) {
            ...
            if (fs.existsSync(file.filePath)) {
              try {
                ...
                const sanitizedEmail = student.email.replace(/[^a-z0-9]/gi, '_');
                const fileName = `${sanitizedEmail}_${student.erpId || 'noID'}_${file.fileName}`;
                
                folder.file(fileName, fileData);
              } catch (error) {
                console.error(`Error reading file for ${student.email}:`, error);
                failedFiles.push({ 
      ...
      try {
        ...
        return next(new HttpError(HttpCode.INTERNAL_SERVER_ERROR, "Failed to generate ZIP file"));
      }
    } catch (error) {
      console.error("Error in batch download endpoint:", error);
      return next(new HttpError(HttpCode.INTERNAL_SERVER_ERROR, "An error occurred while processing your request"));
    }
  })
);
