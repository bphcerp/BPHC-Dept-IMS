...
router.post(
  ...
  asyncHandler((req, res, next) =>
    pdfUpload.fields(phdSchemas.phdProposalMulterFileFields)(req, res, (err) => {
  ...
  asyncHandler(async (req, res) => {
    ...
    const { title } = phdSchemas.phdProposalSubmissionSchema.parse(req.body);
    const userEmail = req.user!.email;

    if (Array.isArray(req.files) || !req.files?.abstractFile || !req.files?.proposalFile) {
      throw new HttpError(HttpCode.BAD_REQUEST, "Both abstract and proposal files are required for resubmission.");
    }
    ...
    await db.transaction(async (tx) => {
      ...
      if (!proposal) {
        throw new HttpError(HttpCode.NOT_FOUND, "Proposal not found or you do not have permission to edit it.");
      }
      if (proposal.status !== "dac_rejected") {
        throw new HttpError(HttpCode.BAD_REQUEST, "This proposal cannot be resubmitted at its current stage.");
      }

      // 1. Upload new files
      const insertedFileIds: { [key: string]: number } = {};
       if (!Array.isArray(req.files) && req.files && Object.entries(req.files).length) {
    ...
    res.status(200).send({ success: true, message: "Proposal resubmitted successfully" });
  })
);
