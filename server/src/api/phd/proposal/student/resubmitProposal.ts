...
router.post(
        ...
        if (err instanceof multer.MulterError) {
          ...
          if (err.code === "LIMIT_FILE_SIZE") {
            userMessage = `File too large. Maximum size is ${
              pdfLimits?.fileSize
                ? (pdfLimits.fileSize / 1024 / 1024).toFixed(1) + "MB"
                : "unknown"
            }.`;
          } else if (err.code === "LIMIT_UNEXPECTED_FILE") {
            userMessage = `Unexpected file field: ${err.field}. Please check uploaded files.`;
          }
          return next(new HttpError(HttpCode.BAD_REQUEST, userMessage));
        } else if (err) {
          logger.error(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (isNaN(proposalId)) {
      return next(new HttpError(HttpCode.BAD_REQUEST, "Invalid proposal ID"));
    }
    ...
    await db.transaction(async (tx) => {
      ...
      if (proposal.status === "draft") {
        if (
          new Date(proposal.proposalSemester.studentSubmissionDate) < new Date()
        ) {
          throw new HttpError(
            HttpCode.FORBIDDEN,
      ...
      const insertedFileIds: Partial<
        Record<(typeof phdSchemas.phdProposalFileFieldNames)[number], number>
      > = {};
      ...
      if (req.files && Object.entries(req.files).length) {
        ...
        const inserted = await tx.insert(files).values(fileInserts).returning();
      ...
      if (submissionType === "final") {
        ...
        nextStatus = proposal.status === "draft" ? "draft" : proposal.status;
      }
      // *** END OF CORRECTION ***
      ...
      await tx
        ...
        .set({
          ...
          placeOfResearchFileId:
            insertedFileIds.placeOfResearchFileId ??
          ...
          status: nextStatus,
          comments: nextStatus === "supervisor_review" ? null : proposal.comments, // Clear comments only on final submit
          updatedAt: new Date(),
        })
    ...
    res.status(200).send({
      success: true,
      message: `Proposal ${
        submissionType === "draft" ? "draft saved" : "resubmitted successfully"
      }.`,
    });
  }),
);
