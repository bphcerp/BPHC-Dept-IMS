// server/src/api/phd/proposal/drcConvener/finalizeProposals.ts
...
export default router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await db
      .update(phdProposals)
      .set({ status: 'sent_to_agsrd' })
      .where(inArray(phdProposals.id, [proposalId]))
      .returning();
    ...
    // Complete the DRC's todo for this proposal
    await completeTodo({
      module: modules[3],
    ...
    res.status(200).json({ success: true, message: 'Proposals finalized.' });
  }),
);