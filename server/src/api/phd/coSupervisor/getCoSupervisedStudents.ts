...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        const students = await db
            ...
            .where(
                or(
                    eq(phd.coSupervisorEmail, coSupervisorEmail),
                    eq(phd.coSupervisorEmail2, coSupervisorEmail)
                )
            );
                    ...
                    applicationStatus:
                        latestApplication.length > 0
                            ? {
                                  ...
                                  comments:
                                      applicationStatusDetails?.comments ??
                                      null,
                                  updatedAs:
                                      applicationStatusDetails?.updatedAs ??
                                      null,
                              }
                            : null,
                    documents,
