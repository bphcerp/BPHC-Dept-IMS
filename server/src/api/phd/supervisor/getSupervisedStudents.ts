...
export default router.get(
    ...
    asyncHandler(async (req, res, next) => {
        ...
        if (!supervisorEmail) {
            return next(
                new HttpError(HttpCode.UNAUTHORIZED, "Unauthorized access")
            );
        }
        ...
        const studentsWithProposals = await Promise.all(
            students.map(async (student) => {
                ...
                return {
                    ...
                    proposalDocuments: proposalDocuments.length
                        ? proposalDocuments
                        : null,
                };
            })
        ...
        const filteredStudents = studentsWithProposals.filter(
            (student) => student !== null
        );

        if (!filteredStudents.length) {
            return next(
                new HttpError(HttpCode.NOT_FOUND, "No proposal documents found")
            );
        }
