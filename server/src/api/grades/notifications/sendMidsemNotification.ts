...
router.post(
    ...
    asyncHandler(async (req, res) => {
        const { courseNames, subject, body } = sendMidsemNotificationSchema.parse(req.body);
        ...
        const filteredStudents = instructorStudents.filter(s =>
            courseNames.includes(s.courseName) && (s.phase === 'draft' || s.phase === 'midsem')
        );

        const draftStudents = filteredStudents.filter(s => s.phase === 'draft');
        const midsemStudents = filteredStudents.filter(s => s.phase === 'midsem');

        if (filteredStudents.length === 0) {
            res.status(400).json({
                success: false,
                message: "No instructor-assigned students found for the selected courses",
            });
            return;
        }
        ...
        const studentsToNotify = midsemStudents.length > 0 ? midsemStudents : filteredStudents;
