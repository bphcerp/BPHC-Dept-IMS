...
console.log("Query result:", request);
    ...
    const fileIds = {
      midSem: request?.midSemFile || null,
      midSemSol: request?.midSemSolFile || null,
      compre: request?.compreFile || null,
      compreSol: request?.compreSolFile || null,
    };
    
    console.log(request)

    // Filter out null values
    const validFileIds = Object.values(fileIds).filter(Boolean) as unknown as number[];
    ...
    const fileUrls = Object.fromEntries(
      Object.entries(fileIds).map(([key, id]) => [key, id ? `${SERVER_URL}/f/${id}` : null])
    );
