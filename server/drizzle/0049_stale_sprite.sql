...
CREATE TABLE "allocation_semester" (
	...
	"created_at" timestamp with time zone DEFAULT now(),
	"updated_at" timestamp with time zone DEFAULT now(),
	CONSTRAINT "allocation_semester_year_odd_even_unique" UNIQUE("year","odd_even")
);
--> statement-breakpoint
...
ALTER TABLE "allocation_form_template_field" ADD CONSTRAINT "allocation_form_template_field_template_id_allocation_form_template_id_fk" FOREIGN KEY ("template_id") REFERENCES "public"."allocation_form_template"("id") ON DELETE cascade ON UPDATE no action;